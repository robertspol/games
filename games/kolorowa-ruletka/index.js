...
function useFetchedData(data) {
    ...
    function draw(i) {
        ...
        wrappers[
            i
        ].style.backgroundColor = `#${colors[red]}${colors[green]}${colors[blue]}`;
    }
    ...
    function play() {
        if (isButtonActive) {
            ...
            if (numbers.moneyAmount < bid)
                return alert(
                    "Podałeś stawkę wyższą od wartości posiadanych pieniędzy!"
                );
            ...
            setTimeout(() => {
                ...
                if (
                    ...
                    (wrapper0 === wrapper1 && wrapper2 === wrapper3) ||
                    (wrapper0 === wrapper2 && wrapper1 === wrapper3)
                ) {
                    wallet(bid, "+", 5);
                } else if (
                    wrapper0 === wrapper1 ||
    ...
    function save() {
        ...
        if (confirmation) {
            fetch("../../php/actions/save-data-kr-action.php", {
                method: "POST",
                headers: {
...
fetch("../../php/actions/get-data-kr-action.php")
    .then((res) => res.json())
